...
def plot_metrics(
    ...
    title: str = "Metric comparison by model",
    xlabel: str = "Model",
) -> None:
...
def plot_distribution(series: pd.Series, title: str, output_path: str | Path | None = None) -> None:
...
def plot_grouped_class_distributions(
    ...
    class_values = sorted({float(label) for dist in distributions.values() for label in dist.index})
    ...
    for idx, (codification, dist) in enumerate(distributions.items()):
        counts_map = {float(label): int(count) for label, count in dist["count"].to_dict().items()}
    ...
    labels = [str(int(value)) if float(value).is_integer() else str(value) for value in class_values]
    plt.title("Class frequencies by GDS codification")
